...
PathAnimation.prototype.stopAnimationNow = function () {		//added this function to stop without invoking pathstop, thus starting animation again.
	if (this.pathPlaying && this.pathPlaying.resets && !this.cancelThread && this.dlyr!=null) this.dlyr.moveTo(this.pathPlaying[0],this.pathPlaying[1]);
	this.stop();
...
PathAnimation.prototype.run = function () {
	...
	//added the next 10 lines for wipeTo()
	var anim = this.pathPlaying;
	if(anim.length==4){//if clip:
		if (anim.frame<anim[0].length) {
			this.dlyr.setClip([anim[0][anim.frame],anim[1][anim.frame],anim[2][anim.frame],anim[3][anim.frame]])
		}else {
			anim.frame = 0;
			this.stopAnimation();
			return;
...
DynLayer.prototype.slideBy = function(x,y,inc,ms) {	// DS: this method is not needed
	if (x==null) x = 0;
	if (y==null) y = 0;
