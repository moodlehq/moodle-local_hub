...
function trans_calculate_users($courseid, $groupid, $savechanges = true) {
    ...
    $navigation = build_navigation(array(array('name' => "Moodle Translators (in $course_name)", 'link' => null, 'type' => 'title')));

    print_header("Moodle Translators (in $course_name)", "Moodle Translators (in $course_name)", $navigation);
    ...
    foreach ($enrolments as $enrolment) {
        ...
        if (!record_exists_sql("SELECT id
                                  FROM {$CFG->prefix}cvsadmin_users
    ...
    print_heading("Translator Members being removed");
