...
define('STATS_CACHE_FILE_NAME_ID', date('Ymd'));
...
function new_registrations_graph() {
    ...
    $sql = "SELECT 
            ...
            FROM (
                ...
                FROM %sregistry
                WHERE timecreated!=0 AND confirmed=1 AND users>0 AND (timeunreachable=0 OR override=1)
            ) AS registry
    ...
    foreach ($month_results as $row) {
        if ($row->Month==date('F') && $row->Year==date('Y')) continue;
...
function all_sites_graph() {
    ...
    $sql = "SELECT
            ...
            LEFT JOIN (
                ...
                FROM `".$CFG->prefix."registry`
                WHERE timecreated!=0 AND timeunreachable!=0 AND confirmed=1 AND users >0
                GROUP BY OrderField
            ) as registry_lost ON registry_lost.OrderField=registry.OrderField
            ORDER BY registry.OrderField ASC";
    $month_results = get_records_sql($sql);
    $thismonth = date('F Y');
    $totalcreatedsofar = 0;
    $totalremovedsofar = 0;
    foreach ($month_results as $row) {
        $totaltoshow = ($totalcreatedsofar-$totalremovedsofar) + (int)$row->created;
	$totalcreatedsofar += (int)$row->created;
        $totalremovedsofar += (int)$row->unreachable;
        $graph->add_value(array(($totalcreatedsofar-$totalremovedsofar), $row->created, substr($row->Month,0,1), $row->Year));
    }
    return $graph;
}
...
function moodle_implementation_map_graph() {
    ...
    $sql = 'SELECT 
            ...
            FROM %sregistry 
            WHERE confirmed=1 AND users>0 AND (timeunreachable=0 OR override=1)
            GROUP BY country 
...
function number_of_users_to_site_size($title, $start=0, $end=1000000, $rounder=4) {
    ...
    $sql = 'SELECT 
            ...
            FROM (
                SELECT `id`, ROUND(`users`, -%d) AS users FROM `%sregistry` WHERE users>%d AND users<=%d AND confirmed=1 AND (timeunreachable=0 OR override=1)
...
function top10_sites_by_users() {
    ...
    $sql = sprintf("SELECT id, users, courses FROM `%sregistry` WHERE confirmed=1 AND (timeunreachable=0 OR override=1) ORDER BY users DESC LIMIT 10", $CFG->prefix);
    $top10results = get_records_sql($sql);
    $count = 1;
    foreach ($top10results as $row) {
        $graph->add_value(array($row->users, $row->courses, (string)$count));
        $count++;
    }
    return $graph;
}
