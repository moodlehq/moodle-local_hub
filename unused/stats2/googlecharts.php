...
define('GOOGLE_CHARTS_COMBINED_TOP_VALUE', 'ctop');
/** Top value for primary range */
define('GOOGLE_CHARTS_PRIMARY_TOP_VALUE', 'one');
...
class google_charts_bar_graph extends bar_graph {
     ...
     * @uses GOOGLE_CHARTS_SECONDARY_TOP_VALUE
     */
    public function get_top_value($setting=null) {
        $topvalues = array();
        foreach ($this->xvalues as $value) {
            for ($i=0;$i < $this->numberofvalues;$i++) {
                if (empty($topvalues[$i]) || (array_key_exists($i, $value->values) && $value->values[$i] > $topvalues[$i])) {
                    $topvalues[$i] = $value->values[$i];
                }
            }
        }
        if ($this->style=='bhg'||$this->style=='bvg') {
            if ($setting !== null) {
                switch ($setting) {
                    case GOOGLE_CHARTS_COMBINED_TOP_VALUE: return max($topvalues);
                    case GOOGLE_CHARTS_PRIMARY_TOP_VALUE: return $topvalues[0];
                    default: return $topvalues[$setting];
                }
            }
            return max($topvalues);
        } else {
            $value = 0;
    ...
    public function generate_url() {
        ...
        if ($this->barlimit!==0) {
           if ($this->limitrtl) {
                $valuecount = count($this->xvalues);
                if ($valuecount > $this->barlimit) {
                    $this->xvalues = array_splice($this->xvalues, count($this->xvalues)-$this->barlimit);
                }
           } else {
                $valuecount = count($this->xvalues);
                if ($valuecount > $this->barlimit) {
                    $this->xvalues = array_splice($this->xvalues, $this->barlimit-1);
                }
           }
        }
        $count = 0;
        $labelsets = array();
        $valuesets = array();
        for ($i = 0; $i < $this->numberofxlabels; $i++) {
            $labelsets[$i] = array();
        }
        for ($i = 0; $i < $this->numberofvalues; $i++) {
            $valuesets[$i] = array();
        ...
        foreach ($this->xvalues as $value) {
            for ($i = 0; $i < $this->numberofxlabels; $i++) {
                if (array_key_exists($i, $value->labels) && $count % $this->xinterval===0) {
                    if (is_array($value->labels[$i])) {
                        foreach ($value->labels[$i] as $l) {
                            $labelsets[$i][] = $l;
                        }
                    } else {
                        $labelsets[$i][] = $value->labels[$i];
                    }
                } else {
                    $labelsets[$i][] = '';
                }
            ...
            for ($i = 0; $i < $this->numberofvalues; $i++) {
                if ($applyscaling) {
                    $valuesets[$i][] = round(($value->values[$i]/$divider)*100,2);
                } else {
                    if (array_key_exists($i, $value->values)) {
                        $valuesets[$i][] = $value->values[$i];
                    }
                }
            }
        }
        ...
        $barcolours = array();
        for ($i = 0; $i < $this->numberofvalues; $i++) {
            if (!array_key_exists($i, $this->ybottoms)) {
                $this->ybottoms[$i] = '0';
            }
            $barcolours[] = $this->barcolours[$i];
        }

        // The Y Axis scale
        $urlbits[] = 'chxr='.count($labelsets).','.$this->ybottoms[0].','.$topvalue.','.ceil($topvalue/$this->ylabelsteps);

        // The colours to use of the bars
        $urlbits[] = 'chco='.join(',', $barcolours);
        ...
        if (preg_match('/b(h|v)g/', $this->style)) {
            $scaling = array();
            for ($i = 0; $i < $this->numberofvalues; $i++) {
                $scaling[] = $this->ybottoms[$i].','.logical_top_point($this->get_top_value($i), $this->logicaltopdefinition);
            }
            $urlbits[] = "chds=".join(',',$scaling);
        }
        ...
        $labels = array();
        for ($i=0;$i < count($labelsets); $i++) {
            $labels[] = $i.':|'.join('|', $labelsets[$i]);
        }
        if ($this->usesecondylabel) {
            // Create the secondary x axis label string
            $labels .= (count($labelsets)+2).':|'.join('|', $secondylabels).'|';
        }
        $labels = 'chxl='.join('|', $labels);
        $urlbits[] = $labels;

        if (count($this->chg)>0) {
            $urlbits[] = 'chg='.join(',', $this->chg);
        }

        $values = array();
        for ($i=0;$i < count($valuesets); $i++) {
            $values[] = join(',', $valuesets[$i]);
        }
