...
function gather_version_information($years=1, $months=0, $days=0) {
    ...
    $sql = 'SELECT COUNT(DISTINCT id) as releasecount, moodlerelease from %sregistry WHERE confirmed=1 AND timecreated>%d AND users>0 GROUP BY moodlerelease';
    $sql = sprintf($sql, $CFG->prefix, $fromtime);
    $resultingversions = get_records_sql($sql);
    if (!is_array($resultingversions)) return false;
    $versions = Array();
...
function download_summary_graph() {
    ...
    $file = $CFG->dirroot.'/'.$currentdir.'/xml/alldownloads_cumu.xml';
    if (!file_exists($file)) {
        return '/'.$currentdir.'/cache/'.$graph->find_latest_cached_graph();
    }
    ...
    for ($i=0;$i<$datacount;$i++) {
        ...
        if ($label2 > $currentyear) {
            $currentyear = $label2;
        } else {
            $label2 = null;
        }
        $graph->add_value(array($label, $label2), array($value));
    }

    $graph->set_grid_lines(round(12/$graph->get_number_of_values()*100,2), 5, 1, 3, round(($monthsin/12)*(12/$graph->get_number_of_values()*100), 2));
 ...
 *                                  the src attribute
 */
function minor_version_pie_graph($versions, $title='Moodle version registrations for 6 months', $filenameprefix='partial') {
    $minorversions = Array();
...
function new_registrations_graph() {
    ...
    $sql = "SELECT 
            ...
            GROUP BY registry.MonthDate
            ORDER BY DateOrder DESC
            LIMIT 37";
    $sql = sprintf($sql, $CFG->prefix);
    $month_results = get_records_sql($sql);
    ...
    foreach ($month_results as $row) {
        ...
        $graph->add_value(array($row->Month, $label2), array($row->Count));
    }
...
function all_sites_graph() {
    ...
    $graph->add_legend('Confirmed registrations');
    $graph->add_legend('New registrations');
    $graph->add_legend('Overall total registrations');
    $graph->set_bar_limit(0);
    $graph->use_second_xlabel();
    $graph->number_of_value_series(3);
    $graph->number_of_xlabels(2);
    ...
    $sql = "SELECT
            ...
            LEFT JOIN (
                ...
                GROUP BY OrderField
            ) as registry_lost ON registry_lost.OrderField=registry.OrderField
            ORDER BY registry.OrderField";
    $month_results = get_records_sql($sql);
    $thismonth = date('F Y');
    $totalcreatedsofar = 0;
    $totalremovedsofar = 0;
	...
	$totalcreatedsofar += (int)$row->created;
        ...
        $graph->add_value(array(substr($row->Month,0,1), $label2), array($totaltoshow, $row->created, $totalremovedsofar));
    }

    $xstep = round((12/$graph->get_number_of_values())*100, 2);
    $xoffset = round(($timeasfirstyear/12)*((12/$graph->get_number_of_values())*100),2);
    $graph->set_grid_lines($xstep, 10, 1, 3, $xoffset, 0);
...
function moodle_implementation_map_graph() {
    ...
    $sql = sprintf($sql, $CFG->prefix);
    $country_results = get_records_sql($sql);
    $count = 0;
    while($count<20) {
        $country=array_shift($country_results);
 ...
 *
 * This function returns a {@link google_charts_bar_graph} graph object that can be used
...
function number_of_users_to_site_size($title, $start=0, $end=1000000, $rounder=4) {
    ...
    $sql = 'SELECT 
            ...
            FROM (
                SELECT `id`, ROUND(`users`, -%d) AS users FROM `%sregistry` WHERE users>%d AND users<=%d AND confirmed=1 AND (timeunreachable=0 OR override BETWEEN 1 AND 3)
            ) AS registry GROUP BY users ORDER BY users';
    $sql = sprintf($sql, $rounder, $CFG->prefix, $start, $end);
    $site_size_results = get_records_sql($sql);
    $xlabel = true;
    $secondxlabelpos = ceil(count($site_size_results)/2);
    $count = 0;
    foreach ($site_size_results as $row) {
        $count++;
        if ($secondxlabelpos==$count) {
            $graph->add_value(array(number_format($row->users), 'Users'), array($row->SiteCount));
        } else {
            $graph->add_value(array(number_format($row->users)), array($row->SiteCount));
        }
    }
    return $graph;
}
...
/**
 * Returns a graph object to display a top 10 sites graph
 *
 * This function returns a {@link google_charts_bar_graph} graph object to display
 * a top 10 sites graph by users (it also displays courses seperately scaled)
 *
 * Use this function in the following way
 * <code>
 * $graph = number_of_users_to_site_size("Large implementations", 10000, 1000000, 5);
 ...
 *
 */
function top10_sites_by_users() {
    global $CFG;

    $graph = new google_charts_bar_graph();
    $graph->set_style(GOOGLE_CHARTS_BAR_VERTICAL_GROUPED);
    $graph->set_chart_title("Top 10 sites by users");
    $graph->set_legend_position(GOOGLE_CHARTS_LEGEND_HBOTTOM);
    $graph->use_second_xvalue();
    $graph->add_legend('Users');
    $graph->add_legend('Courses');
    $graph->logicaltopdefinition = STATS_LOGICALTOP_FIRSTDIGIT;
    $graph->set_group_spacing(20);
    $filename = 'top10_sites.'.STATS_CACHE_FILE_NAME_ID.'.png';
    if (!$graph->set_filename($filename)) {
        // There is an error setting the filename probably that we were unable
        // to create the stats directory in moodledata
    }
    ...
    $sql = sprintf("SELECT id, users, courses FROM `%sregistry` WHERE confirmed=1 AND (timeunreachable=0 OR override BETWEEN 1 AND 3) ORDER BY users DESC LIMIT 10", $CFG->prefix);
    $top10results = get_records_sql($sql);
    $count = 1;
    foreach ($top10results as $row) {
        $graph->add_value(array((string)$count), array($row->users, $row->courses));
        $count++;
    }
    return $graph;
}
...
function moodle_population() {
    ...
    $graph->set_bar_limit(0);
    $graph->number_of_value_series(2);
    $graph->number_of_xlabels(2);
    ...
    $sql = "SELECT
            ...
            FROM (
                SELECT
                    `id`,
                     FROM_UNIXTIME(firstaccess, '%%Y%%m') AS DateOrder,
                     FROM_UNIXTIME(firstaccess, '%%b') AS Month,
                     FROM_UNIXTIME(firstaccess, '%%Y') AS Year
                FROM %suser
                WHERE firstaccess!=0
                ) AS user
    ...
    foreach ($month_results as $row) {
        ...
        $graph->add_value(array(substr($row->Month,0,1), $label2), array($totalsofar, $row->Count));
        $totalsofar += (int)$row->Count;
    }

    $xstep = round((12/$graph->get_number_of_values())*100, 2);
    $xoffset = round(($monthsin/12)*((12/$graph->get_number_of_values())*100),2);
    $graph->set_grid_lines($xstep, 10, 1, 3, $xoffset, 0);
...
function moodle_users_per_site() {
    ...
    $sql = "SELECT count(`id`) AS sitecount FROM `%sregistry` WHERE users>%d AND users<=%d AND confirmed=1 AND (timeunreachable=0 OR override BETWEEN 1 AND 3)";
    ...
    foreach ($range as $key=>$group) {
        $site_size_results = get_record_sql(sprintf($sql, $CFG->prefix, $group['start'], $group['end']));
        $count++;
        $label = number_format($group['start']);
        $graph->overridelabelposition[] = round((100/$xlabelcount)*($key+1), 2);
        if ($key == count($range)-1) {
            $label = array($label, number_format($group['end']+1));
        }
        if ($secondxlabelpos==$count) {
            $graph->add_value(array($label, 'Users'), array($site_size_results->sitecount));
        } else {
            $graph->add_value(array($label), array($site_size_results->sitecount));
        }
    }

    $graph->set_grid_lines(round(100/$xlabelcount, 2), 5);
...
abstract class graph {
    ...
    public function __construct() {
        global $currentdir, $CFG, $forcegeneration;
        $this->webpath = '/'.$currentdir.'/cache/';
        if ($forcegeneration===true) {
...
abstract class bar_graph extends graph {
    /** @var array The bottom y-axis-value for the graph */
    protected $ybottoms = array();
    ...
    protected $barcolours = array('ffcb44', 'f68e00', 'FFEEC0');
    ...
    protected $numberofxlabels = 1;
    protected $numberofylabels = 1;
    protected $numberofvalues = 1;
    ...
    public function set_grid_lines($xstep, $ystep=10, $lw=1, $ls=3, $xoffset=0, $yoffset=0) {
        $this->chg[0] = $xstep;
        $this->chg[1] = $ystep;
        $this->chg[2] = $lw;
        $this->chg[3] = $ls;
     ...
     */
    public function add_value($xlabels, $xvalues) {
        if (!is_array($xvalues)) {
            $xvalues = array($xvalues);
        }
        if (!is_array($xlabels)) {
            $xlabels = array($xlabels);
        }
        $this->xvalues[] = new graph_value($xlabels, $xvalues);
        return true;
    }
    ...
    public function number_of_value_series($num) {
        $this->numberofvalues = $num;
    }

    public function number_of_xlabels($num) {
        $this->numberofxlabels = $num;
    }

    public function get_number_of_values() {
        return count($this->xvalues);
    }
...
class graph_value {
    ...
    public function __construct($labels, $values) {
        if (!is_array($values)) {
            $values = array($values);
        }
        $this->values = $values;
    ...
    public function __get($key) {
        ...
        switch ($key) {
            ...
            case 'label':
                if (array_key_exists(0, $this->labels)) {
                    return $this->labels[0];
                } else {
                    return '';
                }
            case 'secondlabel':
                if (array_key_exists(1, $this->labels)) {
                    return $this->labels[1];
                } else {
                    return '';
                }
            case 'secondvalue':
                if (array_key_exists(1, $this->values)) {
