...
function devel_calculate_users($courseid, $groupid, $savechanges = true) {
    ...
    $navigation = build_navigation(array(array('name' => "Core Moodle Developers (in $course_name)", 'link' => null, 'type' => 'title')));

    print_header("Core Moodle Developers (in $course_name)", "Core Moodle Developers (in $course_name)", $navigation);
    ...
    foreach ($enrolments as $enrolment) {
        ...
        if (!record_exists_sql("SELECT id
                                 ...
                                 WHERE userid = $user->id
                                   AND status = 'approved'
                                   AND ( directories LIKE '%all%' OR directories LIKE '%moodle%')")) {
            continue;
        }
